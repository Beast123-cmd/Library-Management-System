...
export default function EditBookPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const diff = totalCopies - availableCopies; // this is the amount checked out currently
      const newAvailable = Math.max(0, totalCopies - diff);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Edit Book Details</h1>
            <p className="text-slate-400 text-sm">Update the details for "{title}"</p>
          </div>
        </div>
